...
function signStaffToken(user) {
  if (!user) {
    throw new Error('Thiếu thông tin nhân sự');
  }

  if (!toPositiveInteger(user.id)) {
    throw new Error('Mã nhân sự không hợp lệ');
  }

  if (!cleanString(user.username)) {
    throw new Error('Tên đăng nhập nhân sự không hợp lệ');
  }
  ...
  return jwt.sign(
    {
      ...
      fullName: cleanString(user.full_name) || cleanString(user.username),
      role: cleanString(user.role),
      branchId: user.branch_id ? Number(user.branch_id) : null,
      branchName: cleanString(user.branch_name) || null
    },
    getJwtSecret(),
    {
      expiresIn: '8h'
...
function signCustomerToken(customer) {
  ...
  if (!toPositiveInteger(customer.id)) {
    throw new Error('Mã khách hàng không hợp lệ');
  }

  if (!cleanString(customer.name)) {
  ...
  return jwt.sign(
    {
      ...
      username: cleanString(customer.username) || cleanString(customer.phone),
      fullName: cleanString(customer.name),
      role: 'customer',
